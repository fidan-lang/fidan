...
pub fn dispatch(name: &str, args: Vec<FidanValue>) -> Option<FidanValue> {
    match name {
        // ── Range ──────────────────────────────────────────────────────────
        "range" => {
            let start = match args.first() { Some(FidanValue::Integer(n)) => *n, _ => 0 };
            let stop  = match args.get(1)  { Some(FidanValue::Integer(n)) => *n, _ => return Some(FidanValue::Nothing) };
            let step  = match args.get(2)  { Some(FidanValue::Integer(n)) => *n, _ => if stop >= start { 1 } else { -1 } };
            let mut list = FidanList::new();
            if step == 0 { return Some(FidanValue::List(OwnedRef::new(list))); }
            let mut i = start;
        ...
        "setUnion" | "set_union" => {
            if let (Some(FidanValue::Dict(a)), Some(FidanValue::Dict(b))) = (args.first(), args.get(1)) {
                let mut result = FidanDict::new();
                for (k, v) in a.borrow().iter() { result.insert(k.clone(), v.clone()); }
        ...
        "setIntersect" | "set_intersect" => {
            if let (Some(FidanValue::Dict(a)), Some(FidanValue::Dict(b))) = (args.first(), args.get(1)) {
                ...
                for (k, v) in a.borrow().iter() {
                    if b_ref.get(k).is_some() { result.insert(k.clone(), v.clone()); }
                }
                Some(FidanValue::Dict(OwnedRef::new(result)))
            } else { Some(FidanValue::Nothing) }
        }
        "setDiff" | "set_diff" => {
            if let (Some(FidanValue::Dict(a)), Some(FidanValue::Dict(b))) = (args.first(), args.get(1)) {
                ...
                let b_ref = b.borrow();
                for (k, v) in a.borrow().iter() {
                    if b_ref.get(k).is_none() { result.insert(k.clone(), v.clone()); }
                }
                Some(FidanValue::Dict(OwnedRef::new(result)))
            } else { Some(FidanValue::Nothing) }
        }
        ...
        "Queue" => {
            let mut list = FidanList::new();
            if let Some(FidanValue::List(l)) = args.first() {
                for v in l.borrow().iter() { list.append(v.clone()); }
            }
            Some(FidanValue::List(OwnedRef::new(list)))
        }
        "enqueue" => {
        ...
        "dequeue" => {
            if let Some(FidanValue::List(l)) = args.first() {
                ...
                if items.is_empty() { return Some(FidanValue::Nothing); }
                let first = items[0].clone();
                let mut new_list = FidanList::new();
                for item in items.into_iter().skip(1) { new_list.append(item); }
                *borrow = new_list;
                Some(first)
            } else { Some(FidanValue::Nothing) }
        }
        ...
        "pop" => {
            if let Some(FidanValue::List(l)) = args.first() {
                ...
                let items: Vec<FidanValue> = borrow.iter().cloned().collect();
                let len = items.len();
                if len == 0 { return Some(FidanValue::Nothing); }
                let top = items[len - 1].clone();
                let mut new_list = FidanList::new();
                for item in items.into_iter().take(len - 1) { new_list.append(item); }
                *borrow = new_list;
                Some(top)
            } else { Some(FidanValue::Nothing) }
        }
        "stackPeek" | "top" => {
            if let Some(FidanValue::List(l)) = args.first() {
                let borrow = l.borrow();
                let len = borrow.len();
                Some(borrow.get(len.saturating_sub(1)).cloned().unwrap_or(FidanValue::Nothing))
            } else { Some(FidanValue::Nothing) }
        }
            ...
            if let Some(FidanValue::List(outer)) = args.first() {
                ...
                for item in outer.borrow().iter() {
                    if let FidanValue::List(inner) = item {
                        for v in inner.borrow().iter() { result.append(v.clone()); }
                    } else {
        ...
        "zip" => {
            if let (Some(FidanValue::List(a)), Some(FidanValue::List(b))) = (args.first(), args.get(1)) {
                let a_ref = a.borrow();
                let b_ref = b.borrow();
                let len = a_ref.len().min(b_ref.len());
                let mut result = FidanList::new();
                for i in 0..len {
                    let pair = vec![a_ref.get(i).cloned().unwrap_or(FidanValue::Nothing),
                                    b_ref.get(i).cloned().unwrap_or(FidanValue::Nothing)];
                    let mut inner_list = FidanList::new();
                    for v in pair { inner_list.append(v); }
        ...
        "unique" | "dedup" => {
            if let Some(FidanValue::List(l)) = args.first() {
                ...
                for v in l.borrow().iter() {
                    let key = as_str(v);
                    if seen.insert(key) { result.append(v.clone()); }
                }
                Some(FidanValue::List(OwnedRef::new(result)))
            } else { Some(FidanValue::Nothing) }
        }
        "reverse" => {
            if let Some(FidanValue::List(l)) = args.first() {
                ...
                let mut result = FidanList::new();
                for v in items { result.append(v); }
                Some(FidanValue::List(OwnedRef::new(result)))
            } else { Some(FidanValue::Nothing) }
        }
        "sort" => {
            ...
            if let Some(FidanValue::List(l)) = args.first() {
                ...
                items.sort_by(|a, b| {
                    match (a, b) {
                        ...
                        (FidanValue::Float(x), FidanValue::Float(y)) => x.partial_cmp(y).unwrap_or(std::cmp::Ordering::Equal),
        ...
        "concat" => {
            ...
            for arg in &args {
                if let FidanValue::List(l) = arg {
                    for v in l.borrow().iter() { result.append(v.clone()); }
                }
            }
            Some(FidanValue::List(OwnedRef::new(result)))
        }
        "slice" => {
            if let Some(FidanValue::List(l)) = args.first() {
                ...
                let len = items.len();
                let start = match args.get(1) { Some(FidanValue::Integer(n)) => (*n).max(0) as usize, _ => 0 };
                let end   = match args.get(2) { Some(FidanValue::Integer(n)) => (*n as usize).min(len), _ => len };
                let mut result = FidanList::new();
                for v in items[start.min(len)..end.min(len)].iter() { result.append(v.clone()); }
                Some(FidanValue::List(OwnedRef::new(result)))
            } else { Some(FidanValue::Nothing) }
        }
        "first" => {
            match args.first() {
                Some(FidanValue::List(l)) => Some(l.borrow().get(0).cloned().unwrap_or(FidanValue::Nothing)),
        ...
        "sum" => {
            if let Some(FidanValue::List(l)) = args.first() {
                let total: f64 = l.borrow().iter().map(|v| match v {
        ...
        "product" => {
            if let Some(FidanValue::List(l)) = args.first() {
                let prod: f64 = l.borrow().iter().map(|v| match v {
                    FidanValue::Integer(n) => *n as f64,
                    FidanValue::Float(f) => *f,
                    _ => 1.0,
                }).product();
        ...
        "min" => {
            if let Some(FidanValue::List(l)) = args.first() {
                ...
                items.into_iter().reduce(|a, b| {
                    match (&a, &b) {
                        (FidanValue::Integer(x), FidanValue::Integer(y)) => if x <= y { a } else { b },
                        _ => a,
                    }
...
pub fn exported_names() -> &'static [&'static str] {
    &[
        "range",
        "Set", "setAdd", "set_add", "setRemove", "set_remove", "setContains", "set_contains",
