...
*Last updated: 2026-03-05 — Three concurrency features completed. (1) Global `use std.X` namespace fix: `use std.io` / `use std.math` aliases now registered as `MirGlobal`s in pre-pass ④ of `lower.rs`; init fn emits `StoreGlobal`; all named action bodies read via `LoadGlobal`; `io.print()` and `math.sqrt()` work from anywhere. (2) E0401 compile-time parallel data-race detection: new `crates/fidan-passes/src/parallel_check.rs` scans each function for `SpawnConcurrent`/`JoinAll` groups, intersects write-sets with access-sets of module globals, reports races; also checks `ParallelIter` bodies for any `StoreGlobal`; integrated into `emit_mir_safety_diags` helper in `fidan-cli/src/main.rs`; fired for both `Interpret` and `Test` modes. (3) W1004 unawaited `Pending` warning: new `crates/fidan-passes/src/unawaited_pending.rs` tracks spawn destinations vs `AwaitPending` calls; alias-tracing via `copy_of` map + `chase()` eliminates false positives from `var h = spawn ...; await h` SSA copy patterns; `codes.rs` and `explanations.rs` updated. Task name display bug fixed: `ConcurrentBlock` lowering now uses `task.name.unwrap_or(self.new_sym)` so E0401 shows `task 'A'` / `task 'B'` instead of `task 'new'`. All 12 lexer tests + integration suite pass. Previous: 2026-03-05 — Strength reduction + inlining pass, `export use` isolation, Python-style relative imports, `STDLIB_MODULES` simplified to `["std"]`.*
