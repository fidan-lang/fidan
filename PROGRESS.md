...
| Type-check decorator validation (W2004) | ✅ | `check_decorators()` in `fidan-typeck`; W2004 "unknown decorator" for unrecognised names; `@precompile` is the only canonical decorator so far |
...
| LSP server | 🔨 | **Implemented:** hover (type+decl detail), go-to-definition, completion (symbols+keywords+builtins), semantic tokens, formatting, cross-module diagnostics, import chain analysis. **Missing:** `textDocument/signatureHelp`, `textDocument/references`, `textDocument/rename`, `textDocument/codeAction` (fix-it patches), `textDocument/documentSymbol` (outline), inlay hints, folding ranges, context-aware completion (dot-trigger shows all items, not just members) |
| VS Code extension | 🔨 | **Done:** TextMate grammar (`fidan.tmLanguage.json`), `language-configuration.json`, LSP client with format-on-save, `fidan.restartServer` + `fidan.showOutput` commands. **Missing:** snippets file (category declared but no `.json`); `fidan.runFile` command (run current file); status-bar server indicator; completion `insertText` with snippet syntax; constructor calls `Dog()` not highlighted as functions (grammar only matches `[a-z_]` prefix for calls); `Debuggers` category listed in `package.json` but no debug adapter |
...
| Interpreter integration | ✅ | 22/22 passing in `fidan-interp/tests/integration.rs` — empty/arithmetic/action/if/while/attempt/recursive programs + R0001 panic + R9001 parallel fail |
| Static-analysis passes (E0401, W1004, W2006) | ✅ | 5/5 tests + null-safety W2006 warns on `nothing`-operand arithmetic, method calls, field access |
