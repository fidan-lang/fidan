        ...
        for func in &mut prog.functions {
            ...
            for bb in &func.blocks {
                for instr in &bb.instructions {
                    if let Instr::Assign { dest, rhs: Rvalue::Use(src), .. } = instr {
                        copy_map.insert(*dest, src.clone());
                    }
                }
            }
            if copy_map.is_empty() { continue; }
...
fn subst_rvalue(rv: &mut Rvalue, resolve: &impl Fn(&Operand) -> Operand) {
    match rv {
        ...
        Rvalue::Binary { lhs, rhs, .. }  => { subst_op(lhs, resolve); subst_op(rhs, resolve); }
        Rvalue::Unary  { operand, .. }   => subst_op(operand, resolve),
        Rvalue::NullCoalesce { lhs, rhs } => { subst_op(lhs, resolve); subst_op(rhs, resolve); }
        Rvalue::Call { callee, args, .. } => {
            subst_callee(callee, resolve);
            for a in args { subst_op(a, resolve); }
        }
        Rvalue::Construct { fields, .. } => {
            for (_, v) in fields { subst_op(v, resolve); }
        }
        Rvalue::List(elems)   => { for e in elems { subst_op(e, resolve); } }
        Rvalue::Dict(pairs)   => { for (k, v) in pairs { subst_op(k, resolve); subst_op(v, resolve); } }
        Rvalue::Tuple(elems)  => { for e in elems { subst_op(e, resolve); } }
        Rvalue::StringInterp(parts) => {
            for p in parts {
                if let MirStringPart::Operand(op) = p { subst_op(op, resolve); }
            }
        }
...
fn subst_instr(instr: &mut Instr, resolve: &impl Fn(&Operand) -> Operand) {
    match instr {
        ...
        Instr::Call { callee, args, dest: _ , span: _ } => {
            subst_callee(callee, resolve);
            for a in args { subst_op(a, resolve); }
        }
        Instr::SetField { object, value, .. } => { subst_op(object, resolve); subst_op(value, resolve); }
        Instr::GetField { object, .. }       => subst_op(object, resolve),
        Instr::GetIndex { object, index, .. } => { subst_op(object, resolve); subst_op(index, resolve); }
        Instr::SetIndex { object, index, value } => {
            subst_op(object, resolve); subst_op(index, resolve); subst_op(value, resolve);
        }
        Instr::AwaitPending { handle, .. }    => subst_op(handle, resolve),
        Instr::SpawnConcurrent { args, .. }
        | Instr::SpawnParallel  { args, .. }
        | Instr::SpawnExpr      { args, .. } => { for a in args { subst_op(a, resolve); } }
