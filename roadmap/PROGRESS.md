...
| Item | Status | Notes |
|---|---|---|
| `FidanValue` enum | ✅ | Integer, Float, Boolean, Nothing, String, List, Dict, Object, Shared, Function, `EnumVariant { tag, payload: Vec<FidanValue> }`, `ClassType(Arc<str>)` (class-as-value), `StdlibFn(module, name)` |
...
| Enums / ADTs (§22.13) | ✅ | Full ADT support: unit variants (`Direction.North`) and payload variants (`Result.Ok(v)`, `Result.Err(msg)`). `EnumVariantDef { name, payload_types }` in AST; parser handles `Variant(Type, ...)` syntax; typeck stores arity in `EnumInfo`; HIR `EnumDestructure` node + `lower_check_pattern()`; MIR `ConstructEnum`/`EnumTagCheck`/`EnumPayload` Rvalues; interpreter evaluates construction via `dispatch_method_sym` and pattern extraction via `EnumTagCheck`+`EnumPayload`; `check` statement + expression both support payload destructuring bindings. 6 integration tests pass. |
...
| Lambda capture (Phase 2) | ✅ | Outer-scope variable capture fully implemented: `hir_walk_expr` Lambda arm collects free variables from body; HIR `Lambda { params, body, captures }` carries capture list; MIR lowering emits `Rvalue::Capture` for each captured var, passes them into the synthesised function via a `__captures__` local; interpreter binds captured values in closure env at call time. 6 integration tests including multi-var capture pass. |
