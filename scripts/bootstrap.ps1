...
function Download-ResourceToFile {
    param(
        [string]$Url,
        [string]$Destination
    )
...
function Get-Release {
    ...
    $sorted = $releases | Sort-Object -Property @{ Expression = {
...
try {
    ...
    Download-ResourceToFile -Url $archiveUrl -Destination $archivePath
    ...
    if ($firstInstall) {
        ...
        Ensure-PathEntry -CurrentDir (Join-Path $installRootResolved "current")
