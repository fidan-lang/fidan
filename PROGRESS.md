...
| LSP server | ✅ | **All implemented:** hover (type+decl detail), go-to-definition, completion (context-aware dot-trigger showing only members, snippet `insertText`), semantic tokens, formatting, cross-module diagnostics, import chain analysis, `textDocument/signatureHelp`, `textDocument/references`, `textDocument/rename`, `textDocument/codeAction` (fix-it patches from `Diagnostic::data` JSON), `textDocument/documentSymbol` (outline with nested hierarchy), inlay hints (untyped var type annotations), folding ranges (braces, block/line comments) |
