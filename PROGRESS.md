...
| `fidan fmt` formatter | ✅ | `fidan-fmt` crate: `format_source()`, `check_formatted()`, `FormatOptions`; `fidan fmt <file> [--in-place] [--check]` CLI; 18/18 formatter unit tests (incl. round-trip); idempotent on all constructs; `panic(expr)` bug fixed |
