...
pub fn dispatch_op(name: &str, args: Vec<FidanValue>) -> Option<Result<Option<ParallelOp>, String>> {
    match name {
        "parallelMap" | "parallel_map" => {
            ...
            match fn_val {
                ...
                _ => Some(Err("parallelMap requires a function as the second argument".into())),
            }
        }
        "parallelFilter" | "parallel_filter" => {
            ...
            let fn_val = args.into_iter().nth(1).unwrap_or(FidanValue::Nothing);
            match fn_val {
                FidanValue::Function(fid) => Some(Ok(Some(ParallelOp::Filter { list, fn_id: fid }))),
                _ => Some(Err("parallelFilter requires a function as the second argument".into())),
            }
        }
        "parallelForEach" | "parallel_for_each" => {
            ...
            let fn_val = args.into_iter().nth(1).unwrap_or(FidanValue::Nothing);
            match fn_val {
                FidanValue::Function(fid) => Some(Ok(Some(ParallelOp::ForEach { list, fn_id: fid }))),
                _ => Some(Err("parallelForEach requires a function as the second argument".into())),
            }
        }
        "parallelReduce" | "parallel_reduce" => {
            ...
            match fn_val {
                FidanValue::Function(fid) => Some(Ok(Some(ParallelOp::Reduce { list, init, fn_id: fid }))),
                _ => Some(Err("parallelReduce requires a function as the third argument".into())),
            }
        }
        _ => None,
    }
