...
use crate::mir::{
    BasicBlock, Callee, Instr, MirLit, MirProgram, MirStringPart, MirTy, Operand, PhiNode,
    Rvalue, Terminator,
};
...
pub fn print_program(prog: &MirProgram) {
    for func in &prog.functions {
        println!("fn {}  ->  {}", sym_str(func.name.0), fmt_ty(&func.return_ty));
...
fn print_instr(instr: &Instr) {
    match instr {
        ...
        Instr::SetField { object, field, value } => {
            println!("    {}.{} = {}", fmt_op(object), sym_str(field.0), fmt_op(value));
        }
        Instr::GetField { dest, object, field } => {
            println!("    _{} = {}.{}", dest.0, fmt_op(object), sym_str(field.0));
        }
        Instr::GetIndex { dest, object, index } => {
            println!("    _{} = {}[{}]", dest.0, fmt_op(object), fmt_op(index));
        }
        Instr::SetIndex { object, index, value } => {
            println!("    {}[{}] = {}", fmt_op(object), fmt_op(index), fmt_op(value));
        }
        Instr::Drop { local } => {
        ...
        Instr::SpawnConcurrent { handle, task_fn, args } => {
            let args_str = args.iter().map(fmt_op).collect::<Vec<_>>().join(", ");
            println!("    _{} = spawn_concurrent fn{}({})", handle.0, task_fn.0, args_str);
        }
        Instr::SpawnParallel { handle, task_fn, args } => {
            let args_str = args.iter().map(fmt_op).collect::<Vec<_>>().join(", ");
            println!("    _{} = spawn_parallel fn{}({})", handle.0, task_fn.0, args_str);
        }
        Instr::JoinAll { handles } => {
            let hs = handles.iter().map(|h| format!("_{}", h.0)).collect::<Vec<_>>().join(", ");
            println!("    join_all({})", hs);
        }
        Instr::SpawnExpr { dest, task_fn, args } => {
            let args_str = args.iter().map(fmt_op).collect::<Vec<_>>().join(", ");
...
fn print_terminator(term: &Terminator) {
    match term {
        ...
        Terminator::Branch { cond, then_bb, else_bb } => {
            println!("    if {} {{ goto bb{} }} else {{ goto bb{} }}", fmt_op(cond), then_bb.0, else_bb.0);
        }
...
fn fmt_rvalue(rv: &Rvalue) -> String {
    match rv {
        ...
        Rvalue::List(elems)   => {
            format!("[{}]", elems.iter().map(fmt_op).collect::<Vec<_>>().join(", "))
        }
        Rvalue::Dict(pairs)   => {
