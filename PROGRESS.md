...
*Last updated: 2026-03-05 — Three import/display fixes. (1) E0106 "module not found": `collect_file_import_paths()` now returns unresolved imports with spans; both call sites in `fidan-cli/src/main.rs` emit span-annotated `error[E0106]` with fix suggestions. (2) User module imports (`use test2`): four-layer fix across typechecker, HIR lowerer, MIR pre-pass ④, and MIR init fn; interpreter `dispatch_method` routes user-module namespace calls through a `user_fn_map` built at startup. (3) `[value]` display bug: `format_val()` in `fidan-stdlib/src/io.rs` and `fidan-stdlib/src/test_runner.rs` now handles `Namespace` (`<module:X>`), `Function` (`<action#N>`), `Tuple`, `Shared`, and `Pending` — all consistent with `builtins::display()`. All 12 lexer tests + integration suite pass. Previous: 2026-03-05 — Three concurrency features completed (E0401 parallel data-race, W1004 unawaited Pending, task name fix).*
