...
*Last updated: 2026-03-05 — Strength reduction added to `ConstantFolding` (`try_reduce` in `constant_folding.rs`): 14 algebraic identity rules; runs before and after inlining. Inlining pass added (`inlining.rs`): `is_inlinable` + `do_inline`; wired as pass 2 in `run_all`. `run_all` pipeline is now 6 passes: ConstantFolding → Inlining → ConstantFolding → CopyPropagation → DeadCodeElimination → UnreachablePruning. Python-style relative import resolution: `find_relative()` replaces the old `find_in_fidan_modules` walker — no magic folder, mirrors Python package layout (`mod.fdn` or `mod/init.fdn`). `export use` isolation: imports are private by default; `export use std.math` in a module re-exposes the namespace to any importer of that module; `re_export: bool` threaded HIR→MIR→typeck; verified end-to-end. `STDLIB_MODULES` simplified to `["std"]` — only the `std` prefix needs to be excluded from file resolution. Previous: 2026-03-02 — All 21 sections of `test/examples/comprehensive.fdn` now pass correctly. Key fixes this session: (1) `continue` in for-loops caused phi-node `nothing` values — fixed by introducing a dedicated `step_bb` for index increment and tracking all continue-site env snapshots to build proper phi nodes in `step_bb`; (2) typed `catch err -> string` clauses were all lowered as `FidanType::Dynamic` — fixed by reading `CatchClause.ty` in HIR lower via `resolve_type_expr_simple`; (3) parent class fields not accessible in child objects — fixed by including inherited parent fields when building `FidanClass::fields` in `build_class_table`; (4) `lower_attempt` (try/catch/finally) did not create phi nodes for variables modified in catch/otherwise branches — fixed by tracking `(end_bb, env_snapshot)` for each normal-exit path and building phi nodes at `finally_bb`. All existing test files (`test/*.fdn`) continue to pass without regressions.*
